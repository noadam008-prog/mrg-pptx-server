...
async function generatePresentation(config) {
  const { pptxConfig, financialData, coverImageUrl, chartImageUrl, chartData, shopCommentary, monthlyData, execSummary } = config;
  ...
  const CONFIG = {
    ...
    cover: {
      ...
      logoUrl: slideConfig.logo_url || null
    },
    ...
    text: {
      ...
      takeawayTemplate1: textContent.text_takeaway_template_1 || 'Substantial YTD Revenue growth of {var}% driven by market expansion.',
      takeawayTemplate2: textContent.text_takeaway_template_2 || 'Gross Profit margin improved to {var}% due to efficient cost control.',
      takeawayTemplate3: textContent.text_takeaway_template_3 || 'Net Profit increased by ${var}M year-over-year.'
    },
  ...
  function buildWaterfallSvg(v) {
    const W = 1000, H = 736;
    const padL = 24, padR = 24, padT = 56, padB = 76;
    const plotW = W - padL - padR, plotH = H - padT - padB;
    const R = Math.max(0, Number(v.revenue) || 0);
    const C = Math.max(0, Number(v.cogs) || 0);
    const GP = Math.max(0, Number(v.gross_profit) || 0);
    const OX = Math.max(0, Number(v.operating_expenses) || 0);
    const NI = Math.max(0, Number(v.net_income) || 0);
    const maxV = Math.max(R, GP, NI, 1);
    const yOf = (val) => padT + plotH - (val / maxV) * plotH;
    const cats = [
      { label: 'Revenue',      top: R,  bottom: 0,  color: '#' + CONFIG.colors.primary,  value: R },
      { label: 'COGS',         top: R,  bottom: GP, color: '#' + CONFIG.colors.negative, value: -C },
      { label: 'Gross Profit', top: GP, bottom: 0,  color: '#' + CONFIG.colors.gray800,  value: GP },
      { label: 'OPEX',         top: GP, bottom: NI, color: '#' + CONFIG.colors.negative, value: -OX },
      { label: 'Net Income',   top: NI, bottom: 0,  color: '#' + CONFIG.colors.primary,  value: NI },
    ];
    const slot = plotW / cats.length, barW = slot * 0.56;
    const fmtV = (n) => (n < 0 ? '\u2212' : '') + Math.round(Math.abs(n)).toLocaleString('en-US');
    let bars = '', connectors = '', labels = '';
    cats.forEach((c, i) => {
      const bx = padL + i * slot + (slot - barW) / 2;
      const yTop = yOf(c.top), yBot = yOf(c.bottom);
      bars += `<rect x="${bx.toFixed(1)}" y="${yTop.toFixed(1)}" width="${barW.toFixed(1)}" height="${Math.max(2, yBot - yTop).toFixed(1)}" fill="${c.color}" rx="3"/>`;
      labels += `<text x="${(bx + barW / 2).toFixed(1)}" y="${(yTop - 12).toFixed(1)}" text-anchor="middle" font-size="27" font-weight="700" fill="#1F2937">${fmtV(c.value)}</text>`;
      labels += `<text x="${(bx + barW / 2).toFixed(1)}" y="${(padT + plotH + 36).toFixed(1)}" text-anchor="middle" font-size="24" fill="#595959">${c.label}</text>`;
      if (i < cats.length - 1) {
        const nextTopY = yOf(cats[i + 1].top);
        const nextBx = padL + (i + 1) * slot + (slot - barW) / 2;
        connectors += `<line x1="${(bx + barW).toFixed(1)}" y1="${nextTopY.toFixed(1)}" x2="${nextBx.toFixed(1)}" y2="${nextTopY.toFixed(1)}" stroke="#9CA3AF" stroke-width="2.5" stroke-dasharray="7 6"/>`;
      }
    });
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <g font-family="Calibri, Arial, sans-serif">
    <line x1="${padL}" y1="${(padT + plotH).toFixed(1)}" x2="${W - padR}" y2="${(padT + plotH).toFixed(1)}" stroke="#D1D5DB" stroke-width="2"/>
    ${connectors}
    ${bars}
    ${labels}
  </g>
  ...
  function addBudgetChart(slide, pptx, options) {
    const { x, title, subtitle, actual, budget, varPct } = options;
    
    slide.addText(title, {
      x, y: 4.53, w: 2.5, h: 0.3,
      fontSize: CONFIG.fontSizes.table,
      bold: true,
      color: CONFIG.colors.gray800,
      fontFace: CONFIG.fonts.primary
    ...
    slide.addText(subtitle, {
      x, y: 4.78, w: 1.2, h: 0.3,
      fontSize: CONFIG.fontSizes.kpi,
      bold: true,
      color: CONFIG.colors.primary,
      fontFace: CONFIG.fonts.primary
    });
    
    slide.addText(`${varPct >= 0 ? '\u25b2' : '\u25bc'} ${varPct}%`, {
      x: x + 0.46, y: 5.7, w: 0.8, h: 0.25,
      fontSize: CONFIG.fontSizes.table,
      color: varPct >= 0 ? CONFIG.colors.positive : CONFIG.colors.negative,
      fontFace: CONFIG.fonts.primary
    });
    
    slide.addChart(pptx.ChartType.bar, [
      { name: 'Act Mth', labels: [title.split(' -')[0]], values: [actual] },
      { name: 'Budget', labels: [title.split(' -')[0]], values: [budget] }
    ], {
      x: x + 0.46, y: CONFIG.slide4.chartY, w: CONFIG.slide4.chartW, h: CONFIG.slide4.chartH,
      barDir: 'col',
      barGrouping: 'clustered',
      chartColors: [CONFIG.colors.chartBar1, CONFIG.colors.primary],
      showLegend: CONFIG.charts.showLegend,
      legendPos: CONFIG.charts.legendPosition,
      legendFontSize: CONFIG.fontSizes.chartLegend,
      showValAxisTitle: false,
      showCatAxisTitle: false,
      showValue: CONFIG.charts.showValues,
      catAxisLabelColor: 'FFFFFF',
      catAxisLabelFontSize: 1
    });
  }
  ...
  async function addCoverSlide(pptx, inputData, coverImageUrl) {
    ...
    if (CONFIG.cover.logoUrl) {
      const logoData = await downloadImageAsDataUri(CONFIG.cover.logoUrl);
      if (logoData) {
        slide.addImage({ data: logoData, x: 0.35, y: 0.28, w: 1.5, h: 0.5, sizing: { type: 'contain', w: 1.5, h: 0.5 } });
      }
    }
  ...
  try {
    ...
    // SLIDE 2 (v2): EXECUTIVE SUMMARY - rendered only when provided
    const es = (execSummary && typeof execSummary === 'object') ? execSummary : null;
    const esBullets = (es && Array.isArray(es.bullets)) ? es.bullets.filter(Boolean).slice(0, 4) : [];
    if (es && (es.headline || esBullets.length)) {
      const slideES = pptx.addSlide();
      slideES.addText(String(es.headline || 'Executive Summary'), {
        x: 0.5, y: 0.22, w: 9, h: 0.62,
        fontSize: 15, bold: true, color: CONFIG.colors.gray900,
        fontFace: CONFIG.fonts.primary, valign: 'top',
      });
      slideES.addShape(pptx.ShapeType.rect, {
        x: 0.5, y: 0.88, w: 9, h: 0.012,
        fill: { color: 'D9D9D9' }, line: { type: 'none' },
      });
      slideES.addText('Executive Summary', {
        x: 0.5, y: 0.98, w: 9, h: 0.3,
        fontSize: 13, bold: true, color: CONFIG.colors.primary,
        fontFace: CONFIG.fonts.primary,
      });
      const esKpis = [
        { label: 'Revenue', metric: inputData.revenue },
        { label: 'Gross Profit', metric: inputData.gross_profit },
        { label: 'Net Income', metric: inputData.profit_after_tax },
      ];
      esKpis.forEach((k, i) => {
        const cur = Number(k.metric && k.metric.current) || 0;
        const cmp = Number(k.metric && k.metric.comparison) || 0;
        const up = cur >= cmp;
        const tx = 0.5 + i * 3.05;
        slideES.addShape(pptx.ShapeType.rect, {
          x: tx, y: 1.4, w: 2.9, h: 1.15,
          fill: { color: 'F7F8F7' }, line: { color: 'D9D9D9', width: 1 },
        });
        slideES.addShape(pptx.ShapeType.rect, {
          x: tx, y: 1.4, w: 0.06, h: 1.15,
          fill: { color: CONFIG.colors.primary }, line: { type: 'none' },
        });
        slideES.addText(k.label, { x: tx + 0.2, y: 1.5, w: 2.55, h: 0.25, fontSize: 10, color: CONFIG.colors.gray500, fontFace: CONFIG.fonts.primary });
        slideES.addText(formatCurrency(cur), { x: tx + 0.2, y: 1.7, w: 2.55, h: 0.48, fontSize: 20, bold: true, color: CONFIG.colors.primary, fontFace: CONFIG.fonts.primary });
        slideES.addText(`${up ? '\u25b2' : '\u25bc'} ${calculateVariance(cur, cmp)} vs ${String(inputData.comparison_period_label || 'Budget')}`, {
          x: tx + 0.2, y: 2.2, w: 2.55, h: 0.25, fontSize: 9,
          color: up ? CONFIG.colors.positive : CONFIG.colors.negative, fontFace: CONFIG.fonts.primary,
        });
      });
      esBullets.forEach((b, i) => {
        const by = 2.9 + i * 0.62;
        slideES.addShape(pptx.ShapeType.ellipse, {
          x: 0.55, y: by, w: 0.3, h: 0.3,
          fill: { color: CONFIG.colors.primary }, line: { type: 'none' },
        });
        slideES.addText(String(i + 1), { x: 0.55, y: by, w: 0.3, h: 0.3, fontSize: 10, bold: true, color: 'FFFFFF', align: 'center', valign: 'middle', fontFace: CONFIG.fonts.primary });
        slideES.addText(String(b), { x: 1.0, y: by - 0.08, w: 8.4, h: 0.52, fontSize: 11, color: CONFIG.colors.gray800, fontFace: CONFIG.fonts.primary, valign: 'middle' });
      });
    }
    ...
    if (chartImageDataUri) {
      ...
      const cd = chartData;
      const wv = cd.current.values || {};
      const wR = Number(wv.revenue) || 0;
      const wC = Number(wv.cogs) || 0;
      const wGP = (wv.gross_profit !== undefined && wv.gross_profit !== null) ? (Number(wv.gross_profit) || 0) : (wR - wC);
      const wOX = Number(wv.operating_expenses) || 0;
      const wNI = (wv.net_income !== undefined && wv.net_income !== null) ? (Number(wv.net_income) || 0) : (wGP - wOX);
      slide2.addImage({
        data: svgToDataUriCover(buildWaterfallSvg({ revenue: wR, cogs: wC, gross_profit: wGP, operating_expenses: wOX, net_income: wNI })),
        x: CONFIG.slide2.graphX,
        y: CONFIG.layout.contentStartY,
        w: CONFIG.slide2.graphW,
        h: CONFIG.slide2.graphH - 0.24,
      });
      slide2.addText(String(cd.title || `P&L waterfall - ${String(cd.current.label || 'Actual')}`), {
        x: CONFIG.slide2.graphX,
        y: CONFIG.layout.contentStartY + CONFIG.slide2.graphH - 0.2,
        w: CONFIG.slide2.graphW,
        h: 0.2,
        fontSize: CONFIG.fontSizes.small,
        color: CONFIG.colors.gray500,
...
const currentPeriodShort = extractPeriodLabel(inputData.current_period_label);
    ...
    addKeyTakeaways(slide2, pptx, [
      ...
      CONFIG.text.takeawayTemplate3.replace('{var}', netProfitIncrease.toFixed(1))
	    ...
	    addSlideSubtitle(slide3, 'This following slide displays kpis to communicate key financial information to business stakeholders and external stakeholders. It allows you share details such as OPEX ratio, revenue and COGS, taxation expenses, etc.');
...
// Calculate metrics
	    ...
	    // Add gauge charts using helper
	    addGaugeChart(slide3, pptx, { x: 0.5, value: grossProfitMargin, title: 'Gross Profit Margin', color: CONFIG.colors.primary });
	    addGaugeChart(slide3, pptx, { x: 0.5 + CONFIG.slide3.gaugeSpacing, value: opexRatio, title: 'OPEX Ratio', color: CONFIG.colors.primaryDark });
	    addGaugeChart(slide3, pptx, { x: 0.5 + CONFIG.slide3.gaugeSpacing * 2, value: operatingProfitMargin, title: 'Operating Profit Margin', color: CONFIG.colors.primary });
	    addGaugeChart(slide3, pptx, { x: 0.5 + CONFIG.slide3.gaugeSpacing * 3, value: netProfitMargin, title: 'Net Profit Margin', color: CONFIG.colors.primaryDark });

	    // March 2025 label
	    slide3.addText('March, 2025', {
	      x: CONFIG.slide3.tableX,
	      y: 0.75,
	      w: CONFIG.slide3.tableW,
	      h: 0.25,
	      fontSize: 11,
	      bold: true,
	      color: CONFIG.colors.gray800,
	    ...
	    // Income Statement Table (right side)
	    const tableData3 = [];
	    tableData3.push([
	    ...
	    const tableRows3 = [
	      ['Revenue', formatCurrency(inputData.revenue.current)],
	      ['COGS', formatCurrency(inputData.cogs.current)],
	      ['Gross Profit', formatCurrency(inputData.gross_profit.current)],
	      ['OPEX', formatCurrency(inputData.operating_expenses.current)],
	      ['Sales', formatCurrency((Number(inputData.operating_expenses.current) || 0) * CONFIG.opex.salesPct / 100)],
	      ['Marketing', formatCurrency((Number(inputData.operating_expenses.current) || 0) * CONFIG.opex.marketingPct / 100)],
	      ['General & Admin', formatCurrency((Number(inputData.operating_expenses.current) || 0) * CONFIG.opex.adminPct / 100)],
	      ['Other Income', formatCurrency(inputData.other_income.current)],
	      ['Other Expenses', formatCurrency(inputData.finance_costs.current)],
	      ['Operating', formatCurrency(operatingProfit)],
	      ['Add text here', 'Add text here'],
	      ['Add text here', 'Add text here'],
	    ];

	    tableRows3.forEach((row, idx) => {
	      const isBold = idx === 2 || idx === 3 || idx === 9;
	      const bgColor = isBold ? CONFIG.colors.gray100 : CONFIG.colors.white;
	      tableData3.push([
	        { text: row[0], options: { bold: isBold, fontSize: 8, fill: bgColor, color: CONFIG.colors.gray800, fontFace: CONFIG.fonts.primary, align: 'left' } },
	        { text: row[1], options: { bold: isBold, fontSize: 8, fill: bgColor, color: CONFIG.colors.gray800, fontFace: CONFIG.fonts.primary, align: 'right' } },
	      ]);
	    });
	    ...
	    const yearLabelsLegacy = monthLabels.map((m, i) => (i < 10 ? `${m} 2024` : `${m} 2025`));
	    const md = (monthlyData && Array.isArray(monthlyData.labels) && monthlyData.labels.length > 1) ? monthlyData : null;
	    const yearLabels = md ? md.labels.map(String) : yearLabelsLegacy;
	    const nMonths = yearLabels.length;
	    const mdSeries = (key, fb) => (md && Array.isArray(md[key]) && md[key].length === nMonths)
	      ? md[key].map((n) => Math.round(Number(n) || 0)) : fb;

	    const ebitValue = Math.round(operatingProfit);
	    const ebitValues = mdSeries('ebit', new Array(nMonths).fill(ebitValue));
	    const ebitTargetValues = mdSeries('ebit_target', ebitValues.map((v) => Math.round(v * 1.15)));

	    // EBIT Chart
	    slide3.addText('Earning before interest and taxes', {
	      x: CONFIG.slide3.chartEbitX,
	      y: 1.95,
	      w: CONFIG.slide3.chartEbitW,
	      h: 0.2,
	      fontSize: 10,
	      bold: true,
	      color: CONFIG.colors.gray800,
	    ...
	    slide3.addChart(
	      pptx.ChartType.line,
	      [
	        { name: 'EBIT Actual', labels: yearLabels, values: ebitValues },
	        { name: 'EBIT Target', labels: yearLabels, values: ebitTargetValues },
	      ],
	      {
	        x: CONFIG.slide3.chartEbitX,
	        y: CONFIG.slide3.chartEbitY,
	        w: CONFIG.slide3.chartEbitW,
	        h: CONFIG.slide3.chartEbitH,
	        chartColors: [CONFIG.colors.gray900, CONFIG.colors.gray500],
	        lineSize: CONFIG.charts.lineSize,
	        lineDataSymbol: 'circle',
	        lineDataSymbolSize: CONFIG.charts.symbolSize,
	        showLegend: CONFIG.charts.showLegend,
	        legendPos: CONFIG.charts.legendPosition,
	        legendFontSize: CONFIG.fontSizes.chartLegend,
	    ...
	    // OPEX Month to Month
	    const opexTotal = Number(inputData.operating_expenses.current) || 0;
	    const salesValues = mdSeries('opex_sales', new Array(nMonths).fill(Math.round((opexTotal * CONFIG.opex.salesPct) / 100)));
	    const marketingValues = mdSeries('opex_marketing', new Array(nMonths).fill(Math.round((opexTotal * CONFIG.opex.marketingPct) / 100)));
	    const adminValues = mdSeries('opex_admin', new Array(nMonths).fill(Math.round((opexTotal * CONFIG.opex.adminPct) / 100)));

	    slide3.addText('Opex', {
	      x: CONFIG.slide3.chartOpexX,
	      y: 1.95,
	      w: CONFIG.slide3.chartOpexW,
	      h: 0.2,
	      fontSize: 10,
	      bold: true,
	      color: CONFIG.colors.gray800,
	    ...
	    slide3.addChart(
	      pptx.ChartType.bar,
	      [
	        { name: 'Sales', labels: yearLabels, values: salesValues },
	        { name: 'Marketing', labels: yearLabels, values: marketingValues },
	        { name: 'General & Admin', labels: yearLabels, values: adminValues },
	      ],
	      {
	        x: CONFIG.slide3.chartOpexX,
	        y: 2.25,
	        ...
	        barDir: 'col',
	        barGrouping: 'stacked',
	        chartColors: [CONFIG.colors.gray800, CONFIG.colors.primary, CONFIG.colors.chartBar3],
	        showLegend: true,
	        legendPos: 't',
	        legendFontSize: 6,
	    ...
	    // Revenue & COGS
	    const revenueValues = mdSeries('revenue', new Array(nMonths).fill(Math.round(Number(inputData.revenue.current) || 0)));
	    const cogsValues = mdSeries('cogs', new Array(nMonths).fill(Math.round(Number(inputData.cogs.current) || 0)));

	    slide3.addText('Revenue & COGS', {
	      x: CONFIG.slide3.chartEbitX,
	      y: 3.65,
	      w: CONFIG.slide3.chartEbitW,
	      h: 0.2,
	    ...
	    slide3.addChart(
	      pptx.ChartType.bar,
	      [
	        { name: 'Actual Revenue', labels: yearLabels, values: revenueValues },
	        { name: 'COGS', labels: yearLabels, values: cogsValues },
	      ],
	      {
	        x: CONFIG.slide3.chartEbitX,
	        y: CONFIG.slide3.chartRevenueY,
	        w: CONFIG.slide3.chartEbitW,
	        h: CONFIG.slide3.chartEbitH,
	        barDir: 'col',
	        barGrouping: 'clustered',
	        chartColors: [CONFIG.colors.gray900, CONFIG.colors.chartBar2],
	        showLegend: true,
	        legendPos: 't',
	        legendFontSize: 7,
	        valGridLine: { style: 'solid', color: CONFIG.charts.gridColor, size: 1 },
	    ...
	    // OPEX March 2025 (horizontal stacked bar)
	    slide3.addText('Opex', {
	      x: CONFIG.slide3.chartOpexX,
	      y: 3.65,
	      w: CONFIG.slide3.chartOpexW,
	    ...
	    slide3.addChart(
	      pptx.ChartType.bar,
	      [
	        { name: 'Sales', labels: [''], values: [CONFIG.opex.salesPct] },
	        { name: 'Marketing', labels: [''], values: [CONFIG.opex.marketingPct] },
	        { name: 'General & Admin', labels: [''], values: [CONFIG.opex.adminPct] },
	      ],
	      {
	        x: CONFIG.slide3.chartOpexX,
	        y: CONFIG.slide3.chartRevenueY,
	        ...
	        dataLabelColor: CONFIG.colors.white,
	        catAxisLabelColor: CONFIG.colors.white,
	        valAxisLabelColor: CONFIG.colors.white,
	        catAxisLabelFontSize: 1,
	        valAxisLabelFontSize: 1,
	      }
	    );
...
slide4.addText('Amount in JPY', {
	    ...
	    const commentaryRows = Array.isArray(shopCommentary) ? shopCommentary : [];
	    commentaryRows.forEach((row) => {
	      const act = Number(row.actual) || 0;
	      const bud = Number(row.budget) || 0;
	      const variance = act - bud;
	      const variancePct = (row.variance_pct !== null && row.variance_pct !== undefined)
	        ? Number(row.variance_pct).toFixed(1)
	        : (bud !== 0 ? ((variance / bud) * 100).toFixed(1) : '0');
	      const varColor = variance >= 0 ? CONFIG.colors.positive : CONFIG.colors.negative;
	      const varArrow = variance >= 0 ? '\u25b2' : '\u25bc';
	      tableData4.push([
	        { text: String(row.account || ''), options: { fontSize: 8, fontFace: CONFIG.fonts.primary } },
	        { text: String(row.shop || ''), options: { fontSize: 8, fontFace: CONFIG.fonts.primary } },
	        { text: formatCurrency(act), options: { fontSize: 8, fontFace: CONFIG.fonts.primary, align: 'right' } },
	        { text: formatCurrency(bud), options: { fontSize: 8, fontFace: CONFIG.fonts.primary, align: 'right' } },
	        ...
	        { text: String(row.explanation || ''), options: { fontSize: 7, fontFace: CONFIG.fonts.primary, color: CONFIG.colors.gray500 } },
	      ]);
	    });
	    revenueItems.forEach((item, idx) => {
	      const budgetItem = budgetRevenueItems[idx] || { current: 0 };
	      const variance = (Number(item.current) || 0) - (Number(budgetItem.current) || 0);
	      const variancePct = (Number(budgetItem.current) || 0) !== 0 ? ((variance / Number(budgetItem.current)) * 100).toFixed(0) : '0';
	      const varColor = variance >= 0 ? CONFIG.colors.positive : CONFIG.colors.negative;
	      const varArrow = variance >= 0 ? '▲' : '▼';

	      tableData4.push([
	        { text: glAccounts[idx] || '', options: { fontSize: 8, fontFace: CONFIG.fonts.primary } },
	        { text: item.name || '', options: { fontSize: 8, fontFace: CONFIG.fonts.primary } },
	        { text: formatCurrency(item.current), options: { fontSize: 8, fontFace: CONFIG.fonts.primary, align: 'right' } },
	        { text: formatCurrency(budgetItem.current), options: { fontSize: 8, fontFace: CONFIG.fonts.primary, align: 'right' } },
	        { text: formatCurrency(variance), options: { fontSize: 8, fontFace: CONFIG.fonts.primary, align: 'right', color: varColor } },
	        { text: `${varArrow} ${variancePct}%`, options: { fontSize: 8, fontFace: CONFIG.fonts.primary, align: 'right', color: varColor } },
	        { text: 'Explanation for movement\nIpsum lorum\nExplanation for movement', options: { fontSize: 7, fontFace: CONFIG.fonts.primary, color: CONFIG.colors.gray500 } },
	      ]);
	    });
	    ...
	    addBudgetChart(slide4, pptx, {
	      x: CONFIG.slide4.chart2X - 0.46,
	      title: 'Gross Profit - Current Month',
...
app.post('/generate-pptx', async (req, res) => {
  try {
    ...
    // Optional images
    let coverImageUrl = body.coverImageUrl || body.cover_image_url || null;
    const coverPrompt = body.cover_prompt || body.coverPrompt || null;
    if (!coverImageUrl && coverPrompt && process.env.FAL_KEY) {
      try {
        console.log('[FLUX] generating cover image from prompt...');
        const falRes = await axios.post(
          'https://fal.run/fal-ai/flux-pro/v1.1',
          { prompt: String(coverPrompt), image_size: 'landscape_16_9', num_images: 1, safety_tolerance: '2' },
          { headers: { Authorization: `Key ${process.env.FAL_KEY}`, 'Content-Type': 'application/json' }, timeout: 90000 }
        );
        const img = falRes.data && Array.isArray(falRes.data.images) && falRes.data.images[0];
        if (img && img.url) {
          coverImageUrl = img.url;
          console.log('[FLUX] cover image ready');
        }
      } catch (e) {
        console.error('[FLUX] cover generation failed (falling back to plain cover):', e.message);
      }
    } else if (!coverImageUrl && coverPrompt) {
      console.log('[FLUX] cover_prompt received but FAL_KEY not set - using plain cover');
    }
    ...
    const monthlyData = safeJsonParse(body.monthly_data ?? body.monthlyData ?? null, 2);
    const execSummary = safeJsonParse(body.executive_summary ?? body.executiveSummary ?? null, 2);
